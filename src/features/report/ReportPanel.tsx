        ...
        const updated = rows.map((r) =>
          r.id === activeRowId
            ? {
                ...
                specialty: json.data.specialty || r.specialty,
              }
            : r
        );
        saveRows(updated);
  ...
  const handleExportWord = async () => {
    ...
    try {
      ...
      const headerRow = new TableRow({
        tableHeader: true,
        children: ["序号", "问题描述", "图号", "图名", "违反条文", "严重级", "涉及专业"].map(
          (text) =>
            new TableCell({
      ...
      const dataRows = rows.map(
        (r) =>
          new TableRow({
            children: [r.seq.toString(), r.description, r.drawingNo, r.drawingName, r.violation, r.severity, r.specialty].map(
              (text) =>
                new TableCell({
      ...
      const header = ["序号", "问题描述", "图号", "图名", "违反条文", "严重级", "涉及专业"];
      const csvRows = rows.map(r => [r.seq, r.description, r.drawingNo, r.drawingName, r.violation, r.severity, r.specialty]);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="w-20 px-3 py-3 text-xs font-medium text-gray-500 text-center">严重级</th>
                ...
                rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-center">
                      <select
                        value={row.specialty}
                        onChange={(e) => updateField(row.id, "specialty", e.target.value)}
                        className="text-xs px-2 py-1 rounded border border-gray-200 text-gray-600 focus:outline-none focus:border-blue-300 cursor-pointer"
                      >
                        <option value="建筑专业">建筑专业</option>
                        <option value="结构专业">结构专业</option>
                      </select>
                    </td>
                    <td className="px-3 py-3 text-center">
                      <button
